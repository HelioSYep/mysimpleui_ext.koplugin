...
- 原版、细线和圆形三种样式都支持按住滑轨实时拖动；亮度和色温会随手指位置连续调整，松手后确认最终值。
- SimpleUI 与 KOReader 原生窗口使用相同的拖动规则；色温按设备原生范围计算，不再受可见分段按钮档位限制。
- KOReader 原生窗口同步亮度和色温的滑轨、滑块及配套控制格式；设备特有的开关、配置等功能按钮保持不变。
- KOReader 原生窗口在三种样式下都保留亮度和色温的 `Max`；SimpleUI 面板的圆形样式仍会隐藏两者的 `Max`，为滑轨释放空间。
