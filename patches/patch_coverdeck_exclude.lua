...
local function _getExcludePaths(pfx)
    local ok, SUISettings = SimpleUICompat.tryRequire("store")
...
function P.apply()
    ...
    M.build = function(w, ctx)
        ...
        if not source then
            ...
            local ok2, S = SimpleUICompat.tryRequire("store")
    ...
    M.getMenuItems = function(ctx_menu)
        ...
        local ok2, SUISettings = SimpleUICompat.tryRequire("store")
