...
function P.apply()
    ...
    local ok_sts, SUISettings = SimpleUICompat.tryRequire("store")
    ...
    local function _buildSleepCtx()
        ...
        local live_Registry = SimpleUICompat.loaded("registry") or Registry
        local live_Config   = SimpleUICompat.loaded("config") or Config
        ...
        if wants_stats and db_conn then
            local ok_sp, SP = SimpleUICompat.tryRequire("stats_provider")
    ...
    local function _buildHomescreenWidget(screen_w, screen_h, target_page)
        ...
        local live_Registry    = SimpleUICompat.loaded("registry") or Registry
        local live_SUISettings = SimpleUICompat.loaded("store") or SUISettings
        local live_Config      = SimpleUICompat.loaded("config") or Config
